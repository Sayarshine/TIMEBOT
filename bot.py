...
def login_voucher(session_id, voucher):
    ...
    # Real Browser Headers - 403 Forbidden ကိုကာကွယ်ရန်
    headers = {
    ...
    try:
        ...
        response = session.post(post_url, json=data, headers=headers, timeout=15)
        res_text = response.text
        print(f"Server Response Debug (Status: {response.status_code}): {res_text[:200]}")
        
        # If 403, try with different approach
        if response.status_code == 403:
            print("⚠️ 403 Forbidden detected, trying alternative headers...")
            # Alternative headers - simpler version
            alt_headers = {
                "accept": "application/json, text/plain, */*",
                "content-type": "application/json",
                "origin": "https://portal-as.ruijienetworks.com",
            ...
            response = session.post(post_url, json=data, headers=alt_headers, timeout=15)
            res_text = response.text
            print(f"Alternative Response (Status: {response.status_code}): {res_text[:200]}")
        ...
        elif "Forbidden" in res_text or "forbidden" in res_text:
            return None, "Server Blocked (403) - Please try again"
