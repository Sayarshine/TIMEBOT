...
async def handle_key(message):
    global approve
    # chat_id ကို string အနေနဲ့ သေချာစစ်ဆေးနိုင်အောင် ပြင်ဆင်လိုက်သည်
...
async def genkey(message):
    ...
    try:
        ...
        if len(args) < 3:
            ...
            return
        plan = args[1]
        user_id = str(args[2]) # User ID ကို string အဖြစ်သေချာသတ်မှတ်ပေးသည်
        ...
        # auth_list ထဲသို့ အသစ်ထည့်ရန် သို့မဟုတ် Update လုပ်ရန်
        auth_list[user_id] = {
            "expires_at": expiry,
            "plan": plan
        }
...
async def handle_result(message):
    try:
        auth_list, _ = await get_file_content("auth_list.json")
        if str(message.from_user.id) in auth_list:
